...
if (isset($_GET['id'])) {
    ...
    if (isset($_POST['submit'])) {
        if (isset($_POST['status'])) {
            ...
            $update = $conn->prepare("UPDATE contact SET status = :status WHERE id = :id");
            $update->execute([
                ":status" => $status,
                ":id" => $id
            ]);
...
<div class="row">
    <div class="col">
        <div class="card">
            <div class="card-body">
                <h5 class="card-title mb-5 d-inline">Update Contact Submission Status</h5>
                <form method="POST" action="status-contacts.php?id=<?php echo $id; ?>" enctype="multipart/form-data">
